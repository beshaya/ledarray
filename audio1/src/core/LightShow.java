...
public class LightShow extends JFrame{
	...
	boolean enableoutput = true; //enables file writing for communication with processing.
  ...
  final int fftpoints = 256; //8192/8;
  ...
  final TextSlider peakdecay = new TextSlider();
...
private Timer myTimer;
  ...
  public LightShow(){//constructor
    ...
    lfdetector = new PeakDetector(fftdata, 1,5);
    ...
    buttonpanel.add(lfdetector.getControl());
    gain.setPreferredSize(new Dimension(200,800));

    livePat= new Vector<Pattern>();
    livePat.add(new SolidRectPattern(xsize,ysize,fftdata,this));
    livePat.add(new SolidRectPattern(xsize,ysize,fftdata,this));
    livePat.add(new Strobe(xsize,ysize,fftdata,this));

    livePatView.setPatternList(livePat);
    livePatView.setLabel("Live Patterns");

    workPat = new Vector<Pattern>();
    //workPat.add(new SolidRectPattern(xsize,ysize,fftdata,this));
    //workPat.add(new Border(xsize,ysize,fftdata));
    workPat.add(new PLL(xsize,ysize,fftdata,this));
	...
	public void paintComponent ( Graphics g ) {
        ...
        //int fftpoints = 60;
       	for(int i=0;i< fftpoints/2;i++){
       		int linesize = (int)(d.height*Math.log(fftdata[i])/10);
       		g2.fillRect(d.width*i*2/fftpoints, d.height-linesize,5,linesize);
       		//g2.fillRect(d.width*i*2/fftpoints, 0,5,10);
       		//System.out.print(linesize+ " "+fftdata[i]);
       	}
       	//System.out.println();
...
class FrameDriver implements ActionListener{
	  public void actionPerformed(ActionEvent evt) {
	    ...
	    double[] fftchunk = new double[fftpoints];
	    
	    //EXPERIMENT: Calculate the FFT repeatedly to feed peak detector
	    int startoffset;
	    if(newSamples > fftpoints){
	    	startoffset = Math.max(samplelength-newSamples, 0);
	    	newSamples = 0;
		    for(int i =0;i<fftpoints;i++){
		    	fftdata[i] = 0;
		    }
		    
		    for(int offset=startoffset;offset<samplelength-fftpoints;offset+=fftpoints){
			    for(int i=0;i<fftpoints;i++){
			    	fftinput[i] = samples[i+offset] * window[i];
			    }
			    double[] zeroes = new double[fftpoints];
			    fft.fft(fftinput, zeroes);
			    //combine data into power
			    for(int i=0;i<fftpoints/2;i++){
			    	fftchunk[i] = fftinput[i]*fftinput[i] + zeroes[i]*zeroes[i];
			    	//if(fftchunk[i] > fftdata[i]){
			    	fftdata[i] += fftchunk[i];
			    	//}
			    }
			    //System.out.println(fftdata[0]);
	    }
	    ...
	    for(int i=0;i<xsize;i++){
	    	for(int j=0;j<ysize;j++){
	    		Color c = makeColor(combinedFrame[i][j][0]>>2,combinedFrame[i][j][1]>>2,combinedFrame[i][j][2]>>2);
	...
	/**
	 * Write img to D:/java.png, but ONLY if D:/java.lk does not exist
	 * (D:/java.lk exists as a lock file; this program creates it and the consuming client deletes it.)
	 * @param img
	 */
	public static void writeFile(BufferedImage img){
		    try {
		    	File lockfile = new File("D:/java.lk");
